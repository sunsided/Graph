...
namespace Graph
{
	...
	public sealed class DelegateFilter<T> : FilterBase<T, T>
	{
		...
		/// Die zu verwendende Filterfunktion
		/// </summary>
		private readonly Func<DelegateFilter<T>, T, T> _filter;
		...
		/// <param name="filter">The filter.</param>
		/// <param name="tag">Benutzerdefiniertes Tag</param>
		public DelegateFilter(Func<T, T> filter, object tag)
			: this((sender, @in) => filter(@in), tag)
		{
		}
		...
		/// <param name="filter">The filter.</param>
		/// <param name="tag">Benutzerdefiniertes Tag</param>
		public DelegateFilter(Func<DelegateFilter<T>, T, T> filter, object tag)
		{
			Contract.Requires(filter != null);
			_filter = filter;
			Tag = tag;
